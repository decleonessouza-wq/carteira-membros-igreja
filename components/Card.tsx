...
export const Card: React.FC<CardProps> = ({ member, id }) => {
   ...
   */
  const Field = ({ label, value, className = "", center = false }: { label: string, value: string, className?: string, center?: boolean }) => (
    <div className={`relative flex flex-col bg-white border border-green-700 rounded-sm overflow-hidden box-border ${className}`}>
      ...
      <div className="flex-1 flex items-end pb-[2px] px-1 w-full">
        <div className={`w-full text-[10px] font-bold text-gray-900 uppercase font-card leading-tight truncate ${center ? 'text-center' : 'text-left'} pt-2`}>
          {value || "-"}
        </div>
      </div>
    </div>
  ...
  const Watermark = () => (
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0 overflow-hidden">
       <div className="w-[80%] opacity-[0.08] text-green-900 transform scale-110">
         <ChurchLogo />
       </div>
    </div>
  );
    ...
    <div id={id} className="flex flex-col md:flex-row gap-4 items-start justify-center bg-transparent p-1">
      ...
      >
        ...
        <div className="flex items-center p-1 h-[2.0cm] border-b-2 border-green-700 bg-white/95 relative z-10 box-border">
          <div className="w-[1.6cm] h-[1.6cm] flex-shrink-0 text-green-700 ml-1">
            <ChurchLogo />
          </div>
          <Header />
        </div>
        ...
        <div className="h-[0.4cm] bg-green-700 flex items-center justify-center z-10 relative box-border">
           <span className="text-white text-[7px] font-bold uppercase tracking-wider">{CHURCH_DATA.address.split(' - ')[0]} - Rondonópolis/MT</span>
        </div>
      </div>
      ...
      >
        ...
        {/* STAMPS GRID - Updated for Perfect Alignment and Green Borders */}
